...
const ShiftLeadersEfficiencyASide=p=>{
  ...
  const mnt=date.date.split("-")[1];
    ...
    const efficiency=[];
    for (let d of date.shiftLeader.shiftLeader){
        if(d.name===null){
            continue;
        }
        console.log(d.data)
        ...
        const filtredMonth=getEfficiencyMonthSL(d.data, date.month[mnt-1], 'month');
        const efficiencyMonth=getEfficiencyDatas(filtredMonth);
        efficiency.push({
            name:d.name,
            efficiencyDay:efficiencyDay.paidH===0 ? 0 : ((efficiencyDay.prodH/efficiencyDay.paidH)*100).toFixed(2),
            totalTargetDay:efficiencyDay.paidT===0 ? 0 : ((efficiencyDay.prodT/efficiencyDay.paidT)*100).toFixed(2),
            efficiencyMonth:efficiencyMonth.paidH===0 ? 0 : ((efficiencyMonth.prodH/efficiencyMonth.paidH)*100).toFixed(2),
            totalTargetMonth:efficiencyMonth.paidT===0 ? 0 : ((efficiencyMonth.prodT/efficiencyMonth.paidT)*100).toFixed(2)
        })
    }
    const totalEfficiency=[];
    efficiency.forEach(e=>totalEfficiency.push({
        name:e.name,
