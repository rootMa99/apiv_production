...
import { getEfficiencyDatas, getEfficiencyDay, getEfficiencyMonth, getEfficiencyYear, getHC } from "../hooks/getEfficiencyData";

const Efficiency = (p) => {
  const data = useSelector(s=>s.datas);

  const filtredData = getEfficiencyDay(data, "2023-11-27");
  const {prodH:prodHD, paidH:paidHD, prodT:prodTD, paidT:paidTD}= getEfficiencyDatas(filtredData);
  ...
  const {prodH:prodHM, paidH:paidHM, prodT:prodTM, paidT:paidTM}=getEfficiencyDatas(filtredDataMonth);
    ...
    <React.Fragment>
      ...
      <div className={c.efficiency}>
        <div className={c.efficiencyContent}>
          <EfficiencyData title="last day" totalP={totalP} totalT={totalT} gap={gap} />
          <EfficiencyData title="month"  totalP={totalPM} totalT={totalTM} gap={gapM}/>
          <EfficiencyData title="year" totalP={totalPY} totalT={totalTY} gap={gapY}/>
          <EfficiencyData title="hc/day" hc={hc} totalT= {hcTarget} gap={gapHc} />
        </div>
      </div>
    </React.Fragment>
  );
